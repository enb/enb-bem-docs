...
module.exports = require('enb/lib/build-flow').create()
    ...
    .builder(function (files) {
        var examplePattern = this._examplePattern && this.node.unmaskTargetName(this._examplePattern);
        ...
        return vow.all(files.map(function (file) {
                ...
                return vfs.read(filename, 'utf-8')
                    .then(function (source) {
                        ...
                        source = examplePattern ? processBemjsonInSource(source, examplePattern) : source;
...
function processBemjsonInSource(source, examplePattern) {
    return source.replace(BEMJSON_CODE_REGEX, function (match) {
        ...
        return '<!-- bem-example: ' + examplePattern.replace('*', base64) + ' -->';
    });
}
