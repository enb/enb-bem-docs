...
module.exports = require('enb/lib/build-flow').create()
    ...
    .builder(function (files) {
        var exampleDirname = this._examplesPattern && this.node.unmaskTargetName(this._examplesPattern);
        ...
        return vow.all(files.map(function (file) {
                ...
                return vfs.read(filename, 'utf-8')
                    .then(function (source) {
                        ...
                        source = exampleDirname ? processBemjsonInSource(source, exampleDirname) : source;
...
function processBemjsonInSource(source, exampleDirname) {
    return source.replace(BEMJSON_CODE_REGEX, function (match) {
        ...
        return '<!-- bem-example: ' + path.join(exampleDirname, base64) + ' -->';
    });
}
