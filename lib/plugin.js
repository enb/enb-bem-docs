        ...
        _dependOnExamples: function (options) {
            helper.prebuild(function (magic) {
                ...
                if (exampleChannel) {
                    ...
                    var hasOverSets = false;
                    var hasDocSet = false;

                    requiredTargets && requiredTargets.forEach(function (target) {
                        (target.indexOf(options.destPath) === -1) && (hasOverSets = true);
                        (target.indexOf(options.destPath) !== -1) && (hasDocSet = true);
                    });

                    // Запускаем таск сборки примеров, если он не был запущен пользователем
                    if ((!hasOverSets || hasDocSet) &&
                        options.exampleSets &&
